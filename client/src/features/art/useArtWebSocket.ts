...
import { wsHost, wsTokenSuffix } from "../../api/client-base";

function wsUrl(): string {
  const proto = location.protocol === "https:" ? "wss" : "ws";
  // Token in the query string so the server binds the tenant (art generation
  // persists per-tenant); without it writes fall back to tenant_anonymous.
  return `${proto}://${wsHost()}/api/v1/art/ws${wsTokenSuffix()}`;
}
