...
export function useCanvasDocument({
  ...
  // Fallback: if the canvas document fetch hangs (e.g. WebSocket never
  // connects on a slow mobile network), force isLoaded after 6 seconds so
  // the canvas is at least usable empty rather than stuck on a spinner.
  useEffect(() => {
