...
function wsUrl(path: string): string {
  ...
  // Include the auth token so the server binds the tenant; without it the
  // canvas document is read/written in tenant_anonymous.
  return `${proto}://${wsHost()}${path}${wsTokenSuffix()}`;
}
