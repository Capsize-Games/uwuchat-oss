...
export interface ParsedToolCall {
  ...
  rawXml: string;
  /** Index of the first character of the raw <tool_call> block in the
   *  original text — used to interleave tool widgets inline in the
   *  narration. */
  startIndex: number;
  /** Index one past the last character of the raw <tool_call> block. */
  endIndex: number;
}
...
export function parseToolCallContent(text: string): ToolCallParseResult {
  ...
  while ((match = TOOL_CALL_RE.exec(text)) !== null) {
    ...
    // Record the raw block's span in the ORIGINAL text so callers can
    // slice narration segments around it (inline widget placement).
