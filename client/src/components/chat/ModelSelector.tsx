...
export default function ModelSelector() {
  ...
  // Fetch the available local model list on its own so it can be retried
  // independently of the settings read. The list comes over the WebSocket
  // RPC channel, so a transient disconnect (server restart, token refresh)
  // would otherwise leave it permanently empty — the fetch is one-shot and
  // its rejection is swallowed below. We re-run this on every WS reconnect.
  ...
  // The model list arrives over the WS RPC channel. If the initial fetch
  // raced a disconnected socket (e.g. the server was restarting after a DB
  // rebuild), localModels stays empty with no retry. Re-fetch on every
  // (re)connect so the dropdown recovers once the server is reachable.
  useEffect(() => {
    if (isWsConnected()) fetchModels();
  ...
  return (
    <div className={`d-flex align-items-center min-w-0 ${styles.root}`}>
      <ProviderPicker
        value={modelService}
        apiKey={apiKey}
