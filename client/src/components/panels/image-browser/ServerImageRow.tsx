...
import styles from "./ServerImageRow.module.css";
...
export default function ServerImageRow({
  ...
  return (
    <>
      <div
        key={img.id}
        className={`d-flex border rounded p-1 mb-1 ${styles.row}`}
        onContextMenu={handleContextMenu}
      >
        {/* Thumbnail */}
        <div
          className={`border rounded overflow-hidden flex-shrink-0 align-self-start ${styles.thumbnail}`}
        ...
        <div className="ms-2 flex-grow-1 overflow-hidden d-flex flex-column min-w-0">
          <div className="d-flex justify-content-between align-items-start">
            {isEditing ? (
              ...
              <strong
                className={`small ${styles.filename}`}
          ...
          <div className={`small text-muted ${styles.timestamp}`}>
          ...
          <div
            className={`d-flex gap-2 mt-1 border-t-theme flex-wrap ${styles.actions}`}
          >
            <button
              type="button"
              className="icon-btn"
