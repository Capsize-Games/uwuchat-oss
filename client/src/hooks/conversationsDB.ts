 ...
 *
 * The cache is replaced wholesale so it mirrors the server list exactly:
 * the store is cleared first, then repopulated. Without the clear this was
 * upsert-only and stale rows for server-side-deleted conversations were
 * never removed — so deleted conversations would flash on the history panel
 * on every reload (and an empty list could never purge the cache). The
 * clear+put runs in a single transaction so a reader never observes an
 * empty store mid-write. Callers only invoke this after a successful server
 * fetch, so an unreachable server leaves the last-known-good cache intact.
 */
