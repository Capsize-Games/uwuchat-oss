...
start_services() {
    ...
    export PYTHONPATH="${ROOT_DIR}:${ROOT_DIR}/extensions:${ROOT_DIR}/server/src:${ROOT_DIR}/src${PYTHONPATH:+:${PYTHONPATH}}"
    # No hardcoded default: export SPOTIFY_STATE_JWT_SECRET (or set it in the
    # environment) before starting. A baked-in secret was removed for
    # OSS-readiness; the build gate (scripts/build-oss-snapshot.sh) enforces
    # this by hard-failing on hardcoded secret defaults.
    export SPOTIFY_STATE_JWT_SECRET="${SPOTIFY_STATE_JWT_SECRET:-}"
    export SPOTIFY_CLIENT_ID="${SPOTIFY_CLIENT_ID:-}"
    export SPOTIFY_CLIENT_SECRET="${SPOTIFY_CLIENT_SECRET:-}"
    export SPOTIFY_REDIRECT_URI="${SPOTIFY_REDIRECT_URI:-https://localhost:5173/api/v1/spotify/callback}"
