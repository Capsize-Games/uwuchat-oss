...
from airunner_services.conf.model_settings import MODEL_PROVIDERS
...
def run_eval_tests(
    ...
    When ``docker=True``, runs the tests inside the running Docker
    server container via ``docker compose exec``.  The stack must be
    started first with ``./scripts/docker.sh up``.

    Args:
        verbose: Whether to show verbose output
        model: Model ID to use for testing (e.g., 'qwen3.5-9b')
        skip_slow: Skip slow integration tests, run only fast tests
        test_file: Optional specific test file to run
        judge_service: Judge provider service name
        judge_model: Judge model ID
        docker: Run tests inside the Docker server container

    Returns:
        Exit code from pytest
    """
    if docker:
        return _run_eval_tests_in_docker(
            verbose=verbose,
            model=model,
            skip_slow=skip_slow,
            test_file=test_file,
...
def _run_eval_tests_in_docker(
    ...
    """Run eval tests inside the Docker server container.

    Uses ``docker compose exec`` to run pytest inside the already
    running ``server`` container.  The stack must be started first
    with ``./scripts/docker.sh up``.
    """
    import os as _os

    compose_files = ["docker-compose.yml"]
    if _os.path.exists("docker-compose.gpu.yml"):
        compose_files.append("docker-compose.gpu.yml")

    test_target = "server/tests/eval/"
    if test_file:
        test_target = f"server/tests/eval/{test_file}"

    pytest_inner = [
        "python", "-m", "pytest",
        test_target,
        "-p", "no:cacheprovider",
        "--tb=short" if not verbose else "-v",
        "--color=yes",
        "-ra",
        "--timeout=600",
    ]
    if skip_slow:
        pytest_inner.extend(["-m", "not slow"])

    compose_cmd = ["docker", "compose"]
    for cf in compose_files:
        compose_cmd.extend(["-f", cf])

    exec_env: list[str] = []
    if model:
        exec_env.extend(["-e", f"AIRUNNER_EVAL_MODEL={model}"])
    ...
    print(f"Docker eval command: {' '.join(cmd)}")

    return run_command(
        cmd,
        f"Docker-based eval tests"
        + (f" - {test_file}" if test_file else ""),
...
Examples:
  ...
  %(prog)s --unit -v                 Run unit tests with verbose output
    ...
    parser.add_argument(
        ...
        help=(
            "Run eval tests inside the Docker server container "
            "(requires the stack to be running: ./scripts/docker.sh up)"
        ),
    )
    ...
    if (
        args.judge_service
        and args.judge_service not in {ModelService.AIRUNNER.value, ModelService.LOCAL.value}
    ...
    if args.eval or args.all:
        ...
        if not args.docker:
            ...
            # (only relevant for local daemon-based evals)
            kill_stale_servers()
