...
def _eval_latex_expression(
    ...
    try:
        ...
        # Eval-only benchmark-grading helper (server/.../eval/), never
        # imported or reachable outside the benchmark harness. The
        # char whitelist can't spell __class__/__import__ etc., and
        # __builtins__ is emptied, so this isn't a live code-exec path.
        if all(c in "0123456789.+-*/()\nsqrtmath." for c in expr):
            result = eval(  # nosec B307
                expr, {"__builtins__": {}, "math": math}
            )
