...
COHERENCE_PROMPT = """You are an expert evaluator assessing the coherence and logical consistency of an AI character's response.
...
Task: Evaluate whether the character's response is coherent, logically consistent with the conversation so far, and free of garbled, repetitive, or self-contradictory text. Consider:
- Does the response make logical sense given the conversation?
- Is it free of garbled or nonsensical language?
- Is it free of excessive repetition or self-contradiction?
- Does it flow naturally from the previous turns?

NOTE: You are NOT evaluating whether the answer is factually correct or matches a reference answer. You are ONLY evaluating whether the text is coherent and makes logical sense.

Provide your evaluation in the following format:
Score: [0-10, where 0 is completely incoherent/garbled and 10 is perfectly coherent and logical]
Reasoning: [Brief explanation of your score]
...
def create_coherence_evaluator(
    client: JudgeClient,
    model: Optional[str] = None,
) -> LLMAsJudge:
    """Create a coherence evaluator.

    Evaluates whether a response is coherent, logically consistent,
    and free of garbled/repetitive/self-contradictory text.  No
    reference answer is required — the judge scores based on the
    conversation context and the response alone.
    ...
    Returns:
        LLMAsJudge evaluator for coherence
    """
    return LLMAsJudge(
        client=client,
