...
def _default_watch_paths() -> list[Path]:
    ...
    paths: list[Path] = []
    for candidate in ("/app/server/src", "/app/extensions", "/app/projects"):
        p = Path(candidate)
        if p.is_dir():
            paths.append(p)
    if paths:
...
class _ChangeHandler(FileSystemEventHandler):
    ...
    def seed_baselines(self, paths: "list[Path]") -> None:
        """Record current mtimes for all watched ``.py`` files at startup.

        Without this, ``_mtimes`` is empty until the *first* event arrives
        for a given file — and that first event is treated as "first time
        seen" and ignored (see ``dispatch``).  The practical effect is that
        the first real edit of any source file after the server boots is
        swallowed and never triggers a reload, so developers think
        watchdog is broken and restart the container manually.  Seeding the
        baseline up front means the first edit already has a reference
        mtime to compare against and correctly triggers a restart.
        """
        for watch_path in paths:
            if not watch_path.is_dir():
                continue
            for py_file in watch_path.rglob("*.py"):
                if set(py_file.parts) & _IGNORE_PATTERNS:
                    continue
                try:
                    resolved = str(py_file.resolve(strict=False))
                    self._mtimes[resolved] = py_file.stat().st_mtime
                except OSError:
                    continue
    ...
    def dispatch(self, event):
        ...
        if last_mtime is not None and current_mtime == last_mtime:
            # False positive: mtime unchanged (e.g. Docker bind-mount emits a
            # spurious IN_MODIFY when Python imports the file). Ignore.
            return
        # Either the mtime moved forward (a real edit to a known/seeded file)
        # or this is a previously-unseen file (newly created since startup).
        # Both are genuine changes worth reloading for. The startup grace
        # window in the run loop suppresses any import-time noise.
...
class DevReloader:
    ...
    def _start_observer(self) -> None:
        ...
        if not paths:
            ...
            return
        # Seed mtime baselines BEFORE the observer starts so the first real
        # edit of any existing file triggers a reload instead of being
        # silently absorbed as a "first sighting".
        self._handler.seed_baselines(paths)
        observer = Observer()
        for watch_path in paths:
            observer.schedule(self._handler, str(watch_path), recursive=True)
    ...
    def _terminate_child(self) -> None:
        ...
        if self._process.poll() is None:
            logger.info("Stopping server process group (PGID %s)...", pid)
            try:
            ...
            if self._handler.changed:
                ...
                if uptime < _MIN_UPTIME:
                    ...
                    if self._handler.changed:
                        ...
                        continue
                    logger.info(
                        "Source files changed (%s);" " restarting server...",
