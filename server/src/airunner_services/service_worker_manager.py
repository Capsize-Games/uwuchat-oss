...
class ServiceWorkerManager:
    ...
    def llm_generate_worker(self) -> Any:
        ...
        Chooses between :class:`CloudLLMWorker` and :class:`EdgeLLMWorker`
        based on the project model router (when ``AIRUNNER_PROJECT`` is
        set and a ``DIALOGUE`` rule points at a cloud provider).
        Otherwise falls back to the DB ``model_service`` setting.
        """
        if self._llm_generate_worker is None:
            from airunner_services.settings import is_cloud

            if is_cloud():
                from airunner_services.cloud.workers.cloud_llm_worker import (
                    CloudLLMWorker,
                )

                worker_cls: type = CloudLLMWorker
            else:
                ...
                if router is not None and router.rule("DIALOGUE"):
                    provider = router.rule("DIALOGUE").get(
                    ...
                    if provider in (
                        ...
                        worker_cls = CloudLLMWorker
                if worker_cls is None:
                    from airunner_services.edge.workers.edge_llm_worker import (
                        EdgeLLMWorker,
                    )

                    worker_cls = EdgeLLMWorker

            self._llm_generate_worker = self._worker_factory(worker_cls)
    ...
    def sd_worker(self) -> Any:
        ...
        Returns ``None`` in cloud deployments where torch is not available.
        """
