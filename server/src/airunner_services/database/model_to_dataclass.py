...
def _safe_python_type(column) -> type:
    """Return ``column.type.python_type``, falling back to ``object``.

    Some third-party column types (e.g. pgvector ``Vector``) do not
    implement ``python_type`` and raise ``NotImplementedError``.
    """
    try:
...
def model_to_dataclass(model_cls):
    ...
    dataclass_fields = [
        (column.key, _safe_python_type(column), None)
