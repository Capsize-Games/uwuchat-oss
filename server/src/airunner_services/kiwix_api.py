...
# defusedxml guards against XXE / billion-laughs attacks in XML
# fetched from an external service -- the Kiwix catalog is untrusted
# network input, not a file this app controls. It doesn't re-export
# the full ElementTree API (e.g. Element), so ``ET`` above is still
# used for typing/traversal -- only parsing goes through defusedxml.
...
class KiwixAPI:
    ...
    def list_zim_files(
        ...
        try:
            ...
            root = _safe_fromstring(response_text)
