...
class RuntimeMixin(ProcessManagerMixin):
    """Provide bootstrapping helpers."""
    ...
    def _ensure_api_services(self) -> None:
        ...
        if is_cloud():
            self.logger.info(
                "Cloud deployment — skipping edge API service init"
            )
            return
    ...
    def initialize_lifecycle(self, preload_llm: bool = False) -> None:
        ...
        if not is_cloud() and preload_llm:
            lifecycle_service.preload_llm_model()
        if is_cloud():
            self.logger.info(
                "Cloud deployment — LLM worker initialized "
                "(skipping local model preload)"
            )

    def _initialize_knowledge_system(self):
        """Initialize the database-backed knowledge system."""
        ...
        try:
            get_knowledge_base()
            self.logger.info("Knowledge system initialized (DB-backed)")
