...
def extract_geojson_from_pbf(
    ...
    try:
        ...
        for dtype in data_types:
            ...
            if geojson_path.exists() and now - geojson_path.stat().st_mtime < cache_ttl:
...
def download_and_extract(
    ...
    try:
        ...
        if pbf.exists() and now - pbf.stat().st_mtime < cache_ttl:
            ...
            pbf_path = pyrosm.get_data(region, directory=directory, update=True)
        else:
        ...
        logger.exception(f"Failed to download and extract for region {region}: {e}")
        return {
            k: None
            for k in (
                data_types
...
def list_available_regions() -> list[str]:
    ...
    return [name for names in regions.values() for name in names]
