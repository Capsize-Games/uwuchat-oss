...
def get_model_file_size_gb(model_path: str) -> Optional[float]:
    ...
    if path.is_file():
        ...
        # Sum all safetensors, bin, and pt files.  On Python 3.13 the
        # recursive ``**/*`` glob also matches top-level files, so the
        # same file can be yielded by both the flat and the recursive
        # patterns.  Deduplicate to avoid counting each file twice.
        ...
        for pattern in (
            ...
            for file in path.glob(pattern):
                if file.is_file() and file not in counted:
                    counted.add(file)
...
def get_precision_with_vram_estimate(
    ...
    estimate = None
    if model_path:
        estimate = estimate_vram_from_path(model_path, precision, source_precision)
