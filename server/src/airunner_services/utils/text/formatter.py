...
class Formatter:
    """Format content into human-readable output.

    Handles LaTeX for math formulas, Markdown for rich text, and
    plain text.  The output is primarily an image file suitable
    for chatbot display.
    """
    ...
    def _is_markdown(text: str) -> bool:
        """Heuristically check if text contains Markdown syntax.

        Looks for common elements: # (headers), * (lists/emphasis),
        ` (code), []() (links).
        """
        # Headers
        if re.search(
    ...
    def _render_plaintext_to_image(
        ...
        """Render plain text to a PNG image.

        Useful for displaying non-math/non-markdown text as an
        image for consistent output.
        """
        try:
    ...
    def format_content(
        ...
        """Analyze input string, determine format, return formatted result.

        Args:
            content_string: The input string from the LLM.
            output_dir: Directory for generated images (created if needed).

        Returns:
            dict with keys:
                - 'type': 'latex', 'markdown', or 'plaintext'
                - 'output': image path (latex/plaintext) or HTML (markdown)
        ...
        # If neither LaTeX nor Markdown, treat as plain text
        # (do NOT render to image)
        return {
