...
from airunner_services.edge.tts.tts_model_manager import TTSModelManager
from airunner_services.utils.text.language_detection import detect_language
from airunner_services.edge.tts.openvoice_file_checker import (
    should_trigger_openvoice_download,
)
from airunner_services.edge.tts.openvoice_exceptions import (
    OpenVoiceError,
)
...
from airunner_services.edge.tts.openvoice_runtime_helpers import (
...
class OpenVoiceModelManager(TTSModelManager, metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # "tmp.wav" filename under AIRUNNER_BASE_PATH's output dir,
        # not the shared world-writable /tmp directory.
        self.src_path: str = (
            f"{self._output_dir}/tmp.wav"  # nosec B108
        )
    ...
    def source_se(self) -> torch.Tensor:
        if self._source_se is None:
            ...
            self._source_se = torch.load(  # nosec B614 — app-chosen path under AIRUNNER_BASE_PATH
                path,
