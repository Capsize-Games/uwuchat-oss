...
def chat_template_repo_id(adapter: Any) -> str | None:
    ...
    model_id = LLMProviderConfig.resolve_model_id(ModelService.LOCAL.value, model_name)
    if not model_id:
        return None
    model_info = LLMProviderConfig.get_model_info(ModelService.LOCAL.value, model_id)
    if not model_info.get("supports_thinking"):
        return None
    repo_id = str(model_info.get("repo_id") or "").strip()
    if ModelService.QWEN.value not in repo_id.lower():
        return None
