...
def load_quantized_text_encoder(
    ...
    try:
        ...
        if _cache_ready(cache):
            ...
            return CLIPTextModel.from_pretrained(str(cache), device_map="auto")  # nosec B615

        logger.info("Quantizing text_encoder (8-bit) from %s", model_dir)
        model = CLIPTextModel.from_pretrained(  # nosec B615
            model_dir,
...
def load_quantized_text_encoder_2(
    ...
    try:
        ...
        if _cache_ready(cache):
            ...
            return CLIPTextModelWithProjection.from_pretrained(  # nosec B615
                str(cache), device_map="auto"
            )

        logger.info("Quantizing text_encoder_2 (8-bit) from %s", model_dir)
        model = CLIPTextModelWithProjection.from_pretrained(  # nosec B615
            model_dir,
