...
def _flat_dir_size(path: str) -> int:
    """Sum file sizes in a flat (non-recursive) directory."""
    return sum(
...
def _recursive_dir_size(path: str) -> int:
    """Sum file sizes recursively through a directory tree."""
    return sum(
...
def _recursive_file_count(path: str) -> int:
    """Count files recursively through a directory tree."""
    return sum(len(files) for _, _, files in os.walk(path))


def _scan_flat_models(
    base_dir: str, model_type: str, fmt_label: str
) -> list:
    """Scan a flat directory for models and return entries."""
    entries = []
    ...
    for name in os.listdir(base_dir):
        ...
        if not os.path.isdir(model_path):
            continue
        size_gb = _flat_dir_size(model_path) / (1024**3)
        entries.append({
            "name": name,
            "path": model_path,
            "size_gb": size_gb,
...
def _scan_recursive_models(
    base_dir: str, model_type: str, fmt_label: str
) -> list:
    """Scan a directory tree for models and return entries."""
    entries = []
    if not os.path.exists(base_dir):
        return entries
    for name in os.listdir(base_dir):
        ...
        if not os.path.isdir(model_path):
            continue
        size_gb = _recursive_dir_size(model_path) / (1024**3)
        entries.append({
            "name": name,
            "path": model_path,
            "size_gb": size_gb,
            "format": fmt_label,
            "files": _recursive_file_count(model_path),
        })
    return entries


def _scan_llm_models(base_dir: str) -> list:
    """Scan the LLM models directory for GGUF and Safetensor models."""
    entries = []
    if not os.path.exists(base_dir):
        return entries
    for name in os.listdir(base_dir):
        ...
        if gguf_files:
            ...
            continue
        # Safetensor / config-based model directory
        st_files = list(Path(model_path).glob("*.safetensors"))
        cfg_exists = (Path(model_path) / "config.json").exists()
        if st_files or cfg_exists:
            size_gb = _flat_dir_size(model_path) / (1024**3)
            entries.append({
                "name": name,
                "path": model_path,
...
def get_downloaded_models() -> Dict[str, List[Dict]]:
    ...
    Returns:
        Dictionary with model types as keys and lists of
        downloaded model info as values.
    """
    from airunner_services.settings import MODELS_DIR

    downloaded: Dict[str, List[Dict]] = {
        "llm": _scan_llm_models(
            os.path.join(MODELS_DIR, "text/models/llm/causallm")
        ),
        "embedding": _scan_flat_models(
            os.path.join(MODELS_DIR, "text/models/llm/embedding"),
            "embedding", "Safetensors",
        ),
        "art": _scan_recursive_models(
            os.path.join(MODELS_DIR, "art/models"),
            "art", "Diffusion",
        ),
        "tts": _scan_recursive_models(
            os.path.join(MODELS_DIR, "text/models/tts"),
            "tts", "TTS",
        ),
        "stt": _scan_recursive_models(
            os.path.join(MODELS_DIR, "text/models/stt"),
            "stt", "Whisper",
        ),
    }
    return downloaded
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Download full safetensors model instead of GGUF "
            "(larger but higher precision)"
        ),
    )
    ...
    if "/" in args.model:
        print(
            f"\n⚠️  Model '{args.model}' not in catalog, "
            "downloading as custom repository..."
        )
