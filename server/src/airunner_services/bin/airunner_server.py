...
def _validate_host_binding(args: argparse.Namespace) -> bool:
    ...
    Loopback, API-key, and insecure-auth checks are straight-forward.
    Additionally, cloud deployments and project deployments are always
    behind a reverse proxy (Caddy) and use auth extensions, so non-loopback
    binding is safe and expected.
    """
    from airunner_services.api.server import is_loopback_host

    configured_api_key = (os.environ.get("AIRUNNER_API_KEY") or "").strip()
    deployment = os.environ.get("AIRUNNER_DEPLOYMENT", "")
    project = os.environ.get("AIRUNNER_PROJECT", "")
    insecure_env = os.environ.get("AIRUNNER_INSECURE_NO_AUTH", "0") == "1"
    is_cloud = deployment == "cloud" or bool(project)
    if (
        is_loopback_host(args.host)
        or args.insecure_no_auth
        or insecure_env
        or is_cloud
    ):
        return True
...
def _monitor_managed_daemon(client, logger) -> int:
    ...
    Retries transient health-check failures (up to 3 consecutive misses)
    so that brief LLM-request blocks don't kill the daemon process.
    """
    logger.info("Managed daemon ready at %s", client.base_url)
    logger.info("Press Ctrl+C to stop the managed daemon.")
    failures = 0
    while True:
        payload = client._healthcheck_payload(timeout_seconds=10.0)
        if payload is None:
            failures += 1
            if failures < 3:
                logger.warning(
                    "Daemon health check %d/3 failed; retrying...",
                    failures,
                )
                time.sleep(2)
                continue
            logger.error("Daemon became unavailable after 3 failures")
            return 1
        failures = 0
        time.sleep(2)
