...
async def art_daemon_websocket(websocket: WebSocket):
    ...
    if account_id is None:
        ...
        logger.warning(
            "Art daemon WS rejected — missing or invalid token"
        )
        return

    await websocket.accept()
    logger.info(
        "Art daemon WebSocket connected account_id=%s", account_id,
    )
    # Establish JWT→tenant context (WS bypasses HTTP auth middleware) so art
    # generation persistence targets the caller's schema, not tenant_anonymous.
    #
        ...
        await ws.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="art daemon resolve client"
                ),
            }
        )
        return
        ...
        await ws.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="art daemon invoke"
                ),
            }
        )
        return
        ...
        await ws.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="art daemon cancel"
                ),
            }
        )
        ...
        await ws.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="art daemon health"
                ),
            }
        )
        ...
        await ws.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="art daemon unload"
                ),
            }
        )
