...
async def tts_daemon_websocket(websocket: WebSocket):
    ...
    if account_id is None:
        ...
        logger.warning("TTS daemon WS rejected — missing or invalid token")
        return
    await websocket.accept()
    logger.info("TTS daemon WebSocket connected account_id=%s", account_id)
        ...
        await websocket.send_json(
            {
                ...
                "error": log_and_sanitize(
                    exc, logger=logger, context="tts resolve client"
                ),
            }
        )
        return
        ...
        await websocket.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="tts synthesize invoke"
                ),
            }
        )
        return
        ...
        await websocket.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="tts load model"
                ),
            }
        )
        ...
        await websocket.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="tts unload model"
                ),
            }
        )
        ...
        await websocket.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="tts health check"
                ),
            }
        )
        ...
        await websocket.send_json(
            {
                ...
                "status": "failed",
                "error": log_and_sanitize(
                    exc, logger=logger, context="tts cancel"
                ),
            }
        )
