...
def _json_default(value: Any) -> str:
    """Coerce datetimes (and any other odd type) to strings so a single
    non-serializable event payload can't silently kill the drain loop."""
...
class WsEventBus:
    ...
    def broadcast(
        ...
        *,
        ...
        """Push an event to matching subscribers of *event_type*.

        Safe to call from any thread (sync or async).  Dead subscribers
        are cleaned up lazily.

        When *account_id* is provided, only subscribers whose
        ``account_id`` matches receive the event — guarding against
        cross-account data leaks on account-scoped types like
        ``proactive_message``, ``gems_balance``, and
        ``weather_data``.

        When *room_id* is provided, the event is delivered under the
        scoped key ``f"{event_type}:{room_id}"`` so only subscribers
        who specifically registered for that room receive it.
        """
        scoped_type = (
            f"{event_type}:{room_id}" if room_id is not None else event_type
        )
        with self._lock:
            subscribers = list(
                self._subscribers.get(scoped_type, []),
            )

        dead: list[_WsSubscriber] = []
        for sub in subscribers:
            if account_id is not None and sub.account_id != account_id:
                continue
            try:
                sub.enqueue(
                    {
