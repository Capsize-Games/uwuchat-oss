...
async def tts_websocket(websocket: WebSocket):
    ...
    if account_id is None:
        ...
        logger.warning("TTS WS rejected — missing or invalid token")
        return

    await websocket.accept()
    # Bind the JWT→tenant context (WS bypasses HTTP auth middleware) so any
    # per-tenant settings reads target the caller's schema, not anonymous.
