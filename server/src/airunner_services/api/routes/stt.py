            ...
            if model_spec.model_type is ModelType.SPEECH_TO_TEXT:
                models.append(
                    ModelInfo(
                        ...
                        # ModelRegistry is a static catalog and does not
                        # track runtime loaded state.  Edge model managers
                        # (e.g. model_state_mixin.get_model_state()) track
                        # per-model state, but those are not available from
                        # this generic listing endpoint.
                        loaded=False,
                    )
                )
...
async def websocket_transcription(websocket: WebSocket):
    ...
    if account_id is None:
        ...
        logger.warning("STT WS rejected — missing or invalid token")
        return

    await websocket.accept()
    logger.info(
        "STT WebSocket connected account_id=%s", account_id,
    )
