...
# Base directory shared by all accounts.  Individual account subdirectories
# are created under this root so that each account has its own isolated
# image store.  The file watcher monitors *this* root recursively to catch
# changes in all account subdirectories.
_IMAGES_BASE = Path(AIRUNNER_BASE_PATH) / "art" / "other" / "images"
_CACHE_ROOT = Path(AIRUNNER_BASE_PATH) / "cache" / "thumbs"

_THUMB_MAX_SIZE = (200, 200)
_IMAGE_EXTENSIONS = {".png", ".jpg", ".jpeg", ".webp", ".bmp"}

# Allowed root directories for path-safety checks.  The per-account
# subdirectory is resolved at request time and validated against these
# base roots.
_SAFE_ROOTS = (_IMAGES_BASE.resolve(), _CACHE_ROOT.resolve())
...
def _validate_date_component(date_str: str) -> str:
    ...
    :func:`image_validation.validate_date_component`.
    """
    ...
    try:
        return validate_date_component(date_str)
...
def _validate_filename_component(filename: str) -> str:
    ...
    Thin FastAPI-aware wrapper around
    :func:`image_validation.validate_filename_component`.
    """
    from airunner_services.api.routes.image_validation import (
    ...
    try:
        return validate_filename_component(filename)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc


def _get_account_id(request: Request) -> int:
    """Extract the authenticated account ID from the request state.

    Raises ``HTTPException(401)`` when the request is unauthenticated.
    """
...
def _account_images_root(account_id: int) -> Path:
    ...
    Each account has its own subdirectory under the shared images base
    so that one account cannot enumerate or download another account's
    generated images.
    """
    return _IMAGES_BASE / str(account_id)


def _date_dir(account_id: int, date_str: str) -> Path:
    """Return the ``YYYYMMDD`` directory path for a given account
    and date string."""
    return _account_images_root(account_id) / date_str
...
def list_dates(request: Request):
    """List all ``YYYYMMDD`` subdirectories under the account's images.

    The authenticated account ID is derived from ``request.state``
    (set by the auth middleware from the verified JWT) — never from
    a client-supplied query string, path param, or body.
    """
...
def list_images(
    ...
    The authenticated account ID is derived from ``request.state``
    (set by the auth middleware from the verified JWT) — never from
    a client-supplied query string.
    """
    account_id = _get_account_id(request)
    _validate_date_component(date)
    directory = _date_dir(account_id, date)
...
def get_image_info(
    ...
    source = _date_dir(account_id, date) / filename
...
def rename_image(
    ...
    dest = _date_dir(account_id, date) / new_name
    ...
    if dest.is_file():
        ...
        counter = 1
        while dest.is_file():
            dest = _date_dir(account_id, date) / f"{stem} ({counter}){suffix}"
            counter += 1
