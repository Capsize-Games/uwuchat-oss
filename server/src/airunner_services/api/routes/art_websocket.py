...
from airunner_services.utils.error_sanitizer import sanitize_exception_message
...
async def _run_generation(
    ...
    try:
        ...
        await fail_art_job_impl(
            tracker, job_id, sanitize_exception_message(exc)
        )
    finally:
...
async def _handle_cancel(
    ...
    When *account_id* is provided, ownership of the job is verified
    before cancellation — only the job's creator may cancel it.
    """
...
async def art_websocket(websocket: WebSocket):
    ...
    if account_id is None:
        ...
        logger.warning("Art WS rejected — missing or invalid token")
        return

    await websocket.accept()
    logger.info("Art WebSocket connected account_id=%s", account_id)
    # Establish JWT→tenant context (WS bypasses HTTP auth middleware). Art
    # generation persists/reads via background tasks and the SD worker, which
    # inherit this context (asyncio task context + worker enqueue capture);
    # without it those writes land in tenant_anonymous.
    #
    # The DEK is re-resolved per message so it survives cache expiry /
    # server restart (mirrors the HTTP middleware's per-request pattern).
            ...
            while True:
                ...
                with ws_dek_scope(account_id):
                    if msg_type == "generate":
                        job_id = await _handle_generate(
                            websocket, msg, account_id=account_id,
                        )
                        if job_id:
