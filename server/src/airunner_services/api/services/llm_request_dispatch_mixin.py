...
from airunner_services.contract_enums import LLMActionType, ModelService
...
class LLMRequestDispatchMixin:
    ...
    def _default_gguf_runtime_profile(
        ...
        if do_tts_reply and model_service in (None, ModelService.LOCAL.value):
            return "combined_tts"
        return None
    ...
    def _prepare_llm_request(
        ...
        if llm_request.enable_thinking is None:
            llm_request.enable_thinking = (
                LLMRequestDispatchMixin._db_enable_thinking()
            )
        return llm_request

    @staticmethod
    def _db_enable_thinking() -> bool:
        """Read enable_thinking from persisted LLM settings."""
        try:
            ...
            if settings is not None and hasattr(settings, "enable_thinking"):
    ...
    def _build_request_signal_data(
        ...
        data = {
            ...
            # Capture the active tenant and DEK here, while we are still on
            # the request's context. The LLMGenerateWorker processes this
            # in a separate long-lived thread that does not inherit context
            # vars, so it re-applies these before persisting the
            # conversation.
