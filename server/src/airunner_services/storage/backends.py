...
class StorageBackend(ABC):
    ...
    def delete_prefix(self, prefix: str) -> int:
        ...
        Returns the number of objects deleted.  The default
        implementation is a no-op that returns 0; subclasses that
        support bulk enumeration (S3 ``list_objects_v2``, local
        filesystem walk) must override this.
        """
        return 0
    ...
    def resolve_path(self, key: str) -> Path:
        """Resolve a key into a concrete :class:`Path` for containment checks.

        Default raises :class:`NotImplementedError`; subclasses that
        store objects on a local filesystem must implement this so
        callers can perform tenant-subtree containment checks.
        """
        raise NotImplementedError(
            "resolve_path is not available for this backend"
        )
...
class LocalStorageBackend(StorageBackend):
    ...
    def delete_prefix(self, prefix: str) -> int:
        """Delete every file whose key starts with *prefix*.

        Walks the resolved directory tree and removes matching files
        (and empty directories).  Returns the number of files deleted.
        """
        try:
    ...
    def resolve_path(self, key: str) -> Path:
        """Resolve a key to its concrete filesystem path.

        Delegates to :meth:`_resolve` so that the same root-containment
        logic applied during read/write is used for callers performing
        additional tenant-subtree checks.
        """
...
def get_storage_backend() -> StorageBackend:
    ...
    if _backend is None:
        ...
        root = AIRUNNER_BASE_PATH or os.path.expanduser(
