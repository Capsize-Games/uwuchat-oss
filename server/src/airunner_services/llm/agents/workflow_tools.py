...
Available Tools: {
    ', '.join(phase_def.allowed_tools)
    if phase_def.allowed_tools else 'all tools'
}
"""
...
Begin with the {state.current_phase.value} phase. """
"""Use 'get_workflow_status' to check progress."""
...
def transition_phase(
    ...
    try:
        ...
        return (
            ...
            "Use: discovery, planning, execution, review, complete"
        )
...
"""
    ...
    if new_phase.value == "planning":
        next_action = (
            "\nNEXT ACTION: Call add_todo_item(title='...',"
            " description='...') to create your task list.\n"
            "Example: add_todo_item(title='Create hello_world"
...
def start_todo_item(todo_id: str) -> str:
    ...
    if unmet_deps:
        deps = ', '.join(unmet_deps)
        return f"Cannot start '{todo_id}' - waiting on dependencies: {deps}"
...
def complete_todo_item(
    ...
    Args:
        ...
        artifacts_json: Optional JSON object with artifacts
            (e.g., {"file": "path/to/file.py", "tests_passed": true})
    """
    state = get_current_state()
...
def retrieve_artifact(key: str) -> str:
    ...
    if key not in state.artifacts:
        ...
        return f"Artifact '{key}' not found. Available: {avail}"
