...
def enhance_prompt_with_specialized_model(
    ...
    try:
        ...
        enhancement_request = (
            ...
            "Enhance this prompt with rich details, art styles, "
        ...
        if second_prompt:
            second_request = (
                "You are an image generation prompt expert. "
                "Enhance this background/atmosphere prompt with "
                "details about colors, mood, lighting, and "
                "environment.\n\n"
                f"Original: {second_prompt}\n\n"
        ...
        logger.info(
            "Prompt enhanced (prompt_len=%s, enhanced_len=%s, "
            "second_prompt_len=%s, enhanced_second_len=%s)",
            len(prompt or ""), len(enhanced_prompt or ""),
            len(second_prompt or ""), len(enhanced_second or ""),
        )
...
@tool(
    ...
    description=(
        ...
        "Automatically adapts to the current image model's "
        "capabilities."
    ),
    return_direct=True,
    requires_api=True,
    ...
    input_examples=[
        {
            "prompt": (
                "A majestic wolf standing on a mountain peak "
                "at sunset, photorealistic, detailed fur"
            ),
            "second_prompt": (
        ...
        {
            "prompt": (
                "Cute anime girl with blue hair, studio ghibli "
                "style, soft lighting"
            ),
...
def generate_image(
    ...
    if caps.supports_second_prompt and second_prompt:
        ...
        logger.info(
            f"Model '{generator_name}' doesn't support "
            "second_prompt, merging into main prompt"
        )
        prompt = f"{prompt}. {second_prompt}"

    # The calling LLM has already produced a detailed prompt, so skip the
    # specialized-model enhancement: it spins up a SECOND LLM instance
    # (config maps PROMPT_ENHANCEMENT to a full model), which fails to load
    # while the chat model is resident and only adds latency/VRAM pressure.
    logger.info(f"Generating image with {generator_name} ({width}x{height})")
    enhanced_prompt, enhanced_second = prompt, effective_second_prompt
...
@tool(
    ...
    category=ToolCategory.IMAGE,
    description=(
        "Get information about the current image generation "
