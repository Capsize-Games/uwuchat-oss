...
class LLMRequest:
    ...
    # DB ids of knowledge-base documents the user attached to this request;
    # the manager retrieves relevant chunks from pgvector and injects them.
    active_document_ids: Optional[List[int]] = field(default_factory=list)
    ephemeral_conversation: bool = False
    # When True the request bypasses the chat agent entirely: a one-shot,
    # stateless completion that creates no conversation, no chatbot, and
    # persists no messages. Used by utility calls like character generation.
    ...
    def for_action(cls, action: Any) -> "LLMRequest":
        ...
        if action == LLMActionType.DECISION:
            return cls(
                do_sample=True,
                temperature=0.6,
        ...
        if action == LLMActionType.FILE_INTERACTION:
            return cls(
                do_sample=True,
                temperature=0.6,
                repetition_penalty=1.0,
