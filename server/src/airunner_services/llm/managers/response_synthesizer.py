...
class ResponseSynthesizer:
    ...
    def _stream_response_message(
        ...
        synthesis_text = build_tool_result_response_prompt(
            all_tool_content=all_tool_content,
            user_question=user_question,
        )
        prompt = self._build_synthesis_prompt(synthesis_text)
        return self._owner._stream_model_response(prompt, generation_kwargs)

    def _build_synthesis_prompt(self, synthesis_text: str) -> list:
        """Prepend the bot's system prompt so synthesis keeps its voice."""
        system_prompt = getattr(self._owner, "_system_prompt", None)
        msgs: list = []
        if system_prompt:
            msgs.append(SystemMessage(content=system_prompt))
        msgs.append(HumanMessage(content=synthesis_text))
        return msgs
