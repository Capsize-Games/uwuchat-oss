...
def _is_gguf_model(model_path: str) -> bool:
    """Return True if *model_path* points to a GGUF model.

    Imported lazily because the GGUF discovery module lives under
    ``airunner_services.edge`` (has ``torch`` dependency).  In cloud
    deployments the import will fail and we return ``False`` — which is
    always correct since cloud environments never serve GGUF models.
    """
    try:
        import importlib.util
        spec = importlib.util.find_spec(
            "airunner_services.edge.llm.chat_gguf_model_discovery"
        )
        if spec is None:
            return False
        return _is_gguf_model(model_path)
    except ImportError:
        return False
...
class ModelAvailabilityMixin:
    ...
    def _check_model_exists(self: "LLMModelManager") -> bool:
        ...
        if expected_gguf_path is None and _is_gguf_model(model_path):
            self.logger.info(
                "GGUF model found "
                "(preferring GGUF; skipping safetensors validation)"
            )
    ...
    def _trigger_model_download(self: "LLMModelManager") -> bool:
        ...
        if expected_gguf_path is None and _is_gguf_model(self.model_path):
            self.logger.info(
    ...
    def _trigger_gguf_download(self: "LLMModelManager") -> bool:
        ...
        gguf_info = LLMProviderConfig.get_gguf_info(ModelService.LOCAL.value, model_id)
        if gguf_info:
            self.logger.info(
