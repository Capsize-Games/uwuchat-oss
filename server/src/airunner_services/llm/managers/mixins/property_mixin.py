...
try:
    ...
    HardwareProfiler = None  # cloud / non-GPU deployments
...
class PropertyMixin:
        ...
        try:
            ...
            if saved_model_id:
                model_info = LLMProviderConfig.get_model_info(
                    ModelService.LOCAL.value, saved_model_id
                )
                if model_info:
    ...
    def _get_available_vram_gb(self) -> float:
        ...
        Returns:
            Available VRAM in GB, or 0.0 when HardwareProfiler is unavailable
            (cloud deployments, non-GPU environments).
        """
    ...
    def model_name(self) -> str:
        ...
        if requested_model:
            ...
            if resolved_model_id:
                model_info = LLMProviderConfig.get_model_info(
                    ModelService.LOCAL.value, resolved_model_id
                )
                if model_info:
    ...
    def model_path(self) -> str:
        ...
        if self.llm_request is not None and self.llm_request.model:
            ...
            if resolved_model_id:
                ...
                # Path-shaped request — confine to base_path.
                #
                # The request might be an absolute host path (e.g.
                # /home/<user>/.local/share/airunner/...) that gets
                # rewritten below, or a relative traversal like
                # ../../../etc/passwd that must be rejected.
                raw = requested
                base = getattr(
                    self.path_settings, "base_path", None,
                )
                if base:
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Model path '{raw}' is outside the "
                            f"configured AIRUNNER_DATA_DIR "
                            f"({base_path})"
                        ) from None
                model_path = raw
            else:
        ...
        if not model_path and not self.llm_settings.use_local_llm:
            # OpenRouter/Ollama resolve a model name against a remote
            # API — no local GGUF filesystem path is required or
            # meaningful for them.
            return ""
