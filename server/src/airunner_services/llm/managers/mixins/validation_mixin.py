...
from airunner_services.contract_enums import ModelService, SignalCode
...
class ValidationMixin:
    ...
    def _get_repo_id_for_model(self: "LLMModelManager") -> str:
        ...
        if model_id:
            model_info = LLMProviderConfig.get_model_info(
                ModelService.LOCAL.value, model_id
            )
    ...
    def _validate_model_path(self: "LLMModelManager") -> bool:
        ...
        if not self.llm_settings.use_local_llm:
            ...
            # API, not a local GGUF filesystem path — the same
            # exemption _check_and_download_model already applies.
            return True
