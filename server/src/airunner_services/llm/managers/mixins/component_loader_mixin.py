...
class ComponentLoaderMixin:
    ...
    def _load_workflow_manager(self) -> None:
        ...
        try:
            ...
            response_model = self._specialized_chat_models.get(
                "RESPONSE"
            ) if hasattr(self, "_specialized_chat_models") else None
    ...
    def _load_specialized_models(self) -> None:
        """Pre-build chat models for routing keys that differ from DIALOGUE.

        Delegates to :meth:`ModelRouter.build_specialized` so the loop
        body lives in one place shared with the cloud-path counterpart.
        """
