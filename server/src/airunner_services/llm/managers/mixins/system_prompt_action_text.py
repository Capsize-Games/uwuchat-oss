...
ACTION_MODE_PROMPTS = {
    LLMActionType.CHAT: (
        "\n\nMode: CHAT"
        "\nFocus on natural conversation. You may use conversation management"
        " tools (clear_conversation, toggle_tts), the search_news tool"
        " for current events and fact verification, and data storage"
        " tools as needed."
        "\n\nStay focused on the user's current question. "
        "If you receive irrelevant information from tools, set it aside"
    ...
    LLMActionType.PERFORM_RAG_SEARCH: (
        ...
        "\n\nGuidelines: Prefer using the rag_search tool for user"
        " queries when documents are loaded."
        "\n\nWhen the user asks a question:"
        "\n1. Call rag_search(query) first — use the user's query"
        " or a relevant search term"
        "\n2. Answer based on the document excerpts returned"
        "\n3. If rag_search returns no results, explain that no"
        " relevant information was found"
        "\n\nAvailable tools: rag_search (search loaded documents),"
        " search_web (fallback for internet)"
    ),
    LLMActionType.DEEP_RESEARCH: (
        "\n\nMode: DEEP RESEARCH"
        "\n\nYou are conducting comprehensive, multi-source"
        " research. Your goal is to produce a thorough,"
        " well-structured research deliverable with clear"
        " sections, extensive citations, and actionable insights."
        "\n\n**RESEARCH WORKFLOW:**"
        "\n"
        "\n1. **SETUP** (First steps):"
        "\n   - Use `get_current_date_context` to establish"
        " today's date for temporal accuracy"
        "\n   - Clarify the research objective and the output"
        " structure you will deliver in chat"
        "\n"
        ...
        "\n   - Use `validate_content` AFTER scraping to ensure"
        " quality"
        "\n   - Use `validate_research_subject` to verify content"
        " is about the correct subject"
    ...
    LLMActionType.APPLICATION_COMMAND: (
        ...
        "\nYou have access to all tools and should autonomously"
        " determine which tools to use based on the user's"
        " request. Analyze the intent and choose the most"
        " appropriate tools to fulfill the user's needs."
        "\n\n**Guidelines:**"
        "\n1. Focus on answering the user's current question"
        "\n2. If tool results contain irrelevant information,"
        " skip it and focus on relevant data"
        "\n3. If recall_knowledge returns unrelated facts,"
        " prefer search_news or search_web to find the answer"
        "\n4. Avoid tool calls that don't directly address"
        " the user's question"
        "\n5. After gathering information, answer the user's"
        " question directly"
        "\n6. Stay focused on the current topic rather than"
        " carrying over old context"
...
SEARCH_WEB_FORCE_PROMPT = (
    ...
    "\n"
    "\n**Step 2: Scrape**"
    "\n- Call `scrape_website` on 2-3 of the most relevant"
    " URLs for full article content"
    "\n"
    "\n**Step 3: Synthesize**"
...
FORCE_TOOL_INSTRUCTIONS = {
    "search_news": (
        "Search for recent news articles related to the "
        "user's query. Focus on current events and recent "
        "developments."
    ),
    "generate_image": (
    ...
    "rag_search": (
        "Search through the user's uploaded documents for "
        "relevant information. Quote relevant passages and "
        "cite sources."
    ),
    "scrape_website": (
        "Extract and summarize the content from the provided "
        "URL. Focus on the main content and key information."
    ),
    "record_knowledge": (
        "Store the provided information in the knowledge "
        "base. Use an appropriate section for the type of "
        "information."
    ),
    "recall_knowledge": (
        "Search the knowledge base for relevant information "
        "about the query. Return any stored facts that match."
    ),
    "clear_conversation": (
