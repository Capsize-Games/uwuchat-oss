...
from airunner_services.llm.managers.prompt_builder.prompt_assembly import (
...
def get_system_prompt_with_context(
    ...
    """Return the stable system prompt selected for the active tool context.

    Dynamic per-request content (rag_context, datetime, mood) is kept out of
    the system prompt so the provider prefix cache is never invalidated.
    Those pieces are collected by collect_per_turn_context() and injected into
    the human turn in generation_workflow_support.setup_generation_workflow().
    """
    ...
    if force_tool and mode != "conversational":
        base_prompt = base_prompt + get_force_tool_instruction(force_tool)
    return base_prompt
