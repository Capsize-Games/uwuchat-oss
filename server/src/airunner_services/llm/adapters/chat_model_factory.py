"""Factory for creating LangChain ChatModel instances based on settings.

Now delegates to edge/cloud modules for actual model construction.
"""
...
from airunner_services.edge.llm.creation_helper import (
...
class ChatModelFactory:
    ...
    Supports:
    - GGUF local models via llama.cpp (edge)
    ...
    _LOCAL_GGUF_ONLY_MESSAGE = (
        "Local LLM mode requires a GGUF model running through "
        "llama.cpp. Transformers-based local loading and "
        "inference are disabled."
    )
    ...
    def create_from_settings(
        ...
        *max_tokens_override*: per-pipeline max_tokens value.
        """
        del model, tokenizer
        ...
        if provider_runtime.provider == ModelService.LOCAL.value:
...
def _create_provider_model_from_runtime(
    ...
    """Create one provider-backed model or raise when no runtime."""
