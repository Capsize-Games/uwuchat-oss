...
def normalize_tool_payload(data: Any) -> Any:
    ...
    Also unwraps Qwen-family native tool-call payloads: models served
    through the local GGUF daemon (Qwen3.5-9B) emit tool calls as
    ``{"tool_call": {"name": ..., "arguments": {...}}}`` JSON in the
    message content instead of an OpenAI ``tool_calls`` array.  The
    wrapper key is hoisted so ``is_tool_payload`` /
    ``extract_tool_call`` recognize the call exactly like the
    ``{"tool": ...}`` / ``{"name": ...}`` forms.
    """
    if isinstance(data, dict):
        ...
        inner = normalized.get("tool_call")
        if isinstance(inner, dict):
            # Hoist the Qwen wrapper: {"tool_call": {"name", "arguments"}}
            # becomes {"name", "arguments"} at the top level.
            return normalize_tool_payload(inner)
        return normalized
    if isinstance(data, list):
