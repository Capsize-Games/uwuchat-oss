...
DEPLOYMENT = "edge"  # "edge" = local GPU device, "cloud" = SaaS
DEBUG = True
...
# ---- Project (overlay, e.g. "uwuchat") ----
AIRUNNER_PROJECT = ""

# ---- Safety / Content Filtering ----
AIRUNNER_LLM_GUARD_MODEL_PATH = os.path.join(
    AIRUNNER_BASE_PATH, "text", "models", "llm_guard"
)
AIRUNNER_LLM_GUARD_ENABLED = False

# ---- Extensions ----
EXTENSIONS = []  # e.g. ["extensions.auth.config"]

# ---- Billing ----
# Dotted path to a module exposing `get_billing_provider()`, used by
# deployments that ship their own BillingProvider.  Empty keeps the
# shipped UnlimitedBillingProvider (no tiers, no caps).
BILLING_PROVIDER = ""
...
AUTH_REQUIRE_TOS = False  # Set True (via env) to enforce ToS flags at registration
...
AIRUNNER_LLM_OPENROUTER_MODEL = GOOGLE_GEMINI_FLASH_LITE_MODEL
...
AIRUNNER_OLLAMA_BASE_URL = os.environ.get(
    "AIRUNNER_OLLAMA_BASE_URL", "http://localhost:11434"
)
# Selects which backend LLMSettings starts up with (local/openrouter/
# ollama/openai/deepinfra). Empty keeps the desktop-app default of
# local in-process GGUF inference.
AIRUNNER_LLM_PROVIDER = os.environ.get("AIRUNNER_LLM_PROVIDER", "")
# Model name to use for the selected provider (e.g. the Ollama model
# tag). Empty keeps each provider's own hardcoded default.
AIRUNNER_LLM_MODEL = os.environ.get("AIRUNNER_LLM_MODEL", "")
