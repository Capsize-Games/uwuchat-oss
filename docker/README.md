...
First run may prompt for missing credentials (your OpenRouter API key) and
saves them to the project's `.env` automatically — no manual env-file setup.

```bash
./scripts/docker.sh up            # start uwuchat (default; auto-detects GPU)
./scripts/docker.sh up --client   # also start the Vite dev server (:5173)
./scripts/docker.sh up headlesscode   # start the headlesscode stack instead
./scripts/docker.sh down headlesscode  # stop the headlesscode stack
./scripts/docker.sh logs server
```

Every `./scripts/docker.sh` subcommand accepts an optional trailing
`uwuchat`/`headlesscode` selector (default: uwuchat), e.g.
`./scripts/docker.sh logs server headlesscode`.

The API is on http://localhost:8080 (headlesscode: http://localhost:8081).
Source is bind-mounted, so edits to `server/`, `extensions/`, and `client/`
are picked up live.

Default local login (dev stacks): `admin@example.com` / `admin123`
(superuser) and `user@example.com` / `user1234` — auto-created by the initial
migration on first boot when no accounts exist (development mode only).
...
AI Runner home in your project env file (`projects/uwuchat/.env`):

```bash
# projects/uwuchat/.env
...
runtime and enabled via `AIRUNNER_EXTENSIONS` in your project env file
(`projects/uwuchat/.env`). No private code is
