...
export function AvatarStep({ onSkip }: Props) {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      uploadProfileImage("avatar", base64).then((result) => { setUploading(false); if (result) setSaved(true); }).catch(() => setUploading(false));
    };
  ...
  return (
    <div className={styles.wrap}>
      <p className={`small ${styles.desc}`}>{t("setup.steps.avatar_desc")}</p>
      <div onClick={() => fileRef.current?.click()} className={`${styles.avatarUpload} ${preview ? styles.uploadPreview : ""}`}
        style={preview ? { "--preview-url": `url(${preview}) center/cover` } as React.CSSProperties : undefined}>
        {!preview && <span className={styles.uploadIcon}>📷</span>}
      </div>
      ...
      <div className={styles.btnRowCenter}>
        <WizardButton variant="primary" size="lg" onClick={() => fileRef.current?.click()} disabled={uploading}>
          {uploading ? t("setup.steps.uploading") : saved ? "✓ " + t("setup.steps.upload_avatar") : t("setup.steps.upload_avatar")}</WizardButton>
