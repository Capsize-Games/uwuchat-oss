...
function SectionLabel({ label }: { label: string }) {
  return <div className={styles.sectionLabel}><div className={styles.sectionLabelInner}>{label}</div></div>;
}
  ...
  return (
    <div className={`d-flex flex-column overflow-hidden ${styles.wrap}`}>
      <div className="scroll-panel">
        ...
        : <>
          {systemBots.length > 0 && <><SectionLabel label={t("sidebar.assistant")} />{systemBots.map(renderRow)}</>}
          {personas.length > 0 && <>{systemBots.length > 0 && <SectionLabel label={t("sidebar.contacts", "Contacts")} />}{active.map(renderRow)}{deceased.length > 0 && <><SectionLabel label={t("sidebar.deceased")} />{deceased.map(renderRow)}</>}</>}
        </>}
      </div>
    </div>
  );
