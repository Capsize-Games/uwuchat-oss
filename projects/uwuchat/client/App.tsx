...
function AppBody() {
  ...
  // settings, subscribe) are rendered as siblings of <Routes> below, so
  ...
  // The wizard stays visible until the user completes every profile
  ...
  // so that a stale pricing_declined flag never reopens the wizard
  // after the user has subscribed and completed setup.
  ...
  const showWizard =
    ...
    (
      forceWizard ||
      (needsSetup && (pricingDeclined ? !wizardDismissed : true))
    );

  const handleSubscribe = useCallback(async (_tier: string) => {}, []);
  ...
  const handleDeclinePricing = useCallback(async () => {
    await declinePricing();
  ...
  return (
      ...
      <Route
        ...
        element={
          <HomeRoute
            ...
            wizardDone={!!setupUser?.setup_complete}
          />
        }
      />
    ...
    {showWizard && (
      <SetupWizard
        ...
        onDeclinePricing={handleDeclinePricing}
      />
    )}
...
function HomeRoute({
  ...
  return (
    <OnboardingTourController enabled={wizardDone}>
      <Layout>
        <MainContent
          chatbotId={chatbotId}
