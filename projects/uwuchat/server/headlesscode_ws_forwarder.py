"""In-process forwarder: headlesscode Redis outbox → WebSocket push.

The Celery worker that polls headlesscode sessions writes discovered
events to a Redis outbox (``headlesscode_event_store``) because it
...
``email/sync_progress_events.py``'s comment on exactly this).
...
``headlesscode_session`` event type, scoped by ``account_id`` so a
subscriber only ever receives its own sessions' events.
"""
...
from projects.uwuchat.server.headlesscode_event_store import (
    headlesscode_events_drain,
)

logger = logging.getLogger(__name__)

# How often to poll the outbox. Sub-second latency isn't needed for
# session cards; 0.5s keeps the loop cheap.
    ...
    while True:
        try:
            payloads = await loop.run_in_executor(
                None, headlesscode_events_drain, _DRAIN_BATCH,
            )
            ...
            logger.debug(
                "Headlesscode forward iteration failed", exc_info=True,
            )
        await asyncio.sleep(_DRAIN_INTERVAL_SECONDS)


def start_headlesscode_ws_forwarder() -> asyncio.Task:
    """Launch the forward loop as a background task."""
    return asyncio.create_task(_forward_loop())


__all__ = ["start_headlesscode_ws_forwarder"]
